...
from one.views_constant import ALL_TYPE, ORDER_TOTAL, ORDER_PRICE_UP, ORDER_PRICE_DOWN, ORDER_SALE_UP, ORDER_SALE_DOWN, \
    HTTP_OK, HTTP_USER_EXIST, ORDER_STATUS_NOT_SEND, ORDER_STATUS_NOT_PAY
from one.views_helper import hash_str, send_email_activate, get_total_price
...
def login(request):
    if request.method == "GET":
        ...
        try:
            ...
            if AXFUser.objects.get(u_username=username):
                password = hash_str(password)
                if password == user.u_password:
                    if user.is_active:
                        ...
                        print("未激活")
                        request.session['error_message'] = '账户未激活'
                        return redirect(reverse('axf:login'))
                else:
                    print("密码错误")
                ...
                request.session['error_message'] = '账户不存在!'
                return redirect(reverse('axf:login'))
